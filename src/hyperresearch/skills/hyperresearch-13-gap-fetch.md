...
4. **Run targeted fetch wave.** For each gap, generate 2-3 search queries and collect URLs. Spawn **2-4 fetchers** with the gap-filling URLs.
   ...
   **Exa-aware search hint.** When you craft each search query for a fetcher, also tell the fetcher which Exa mode to prefer based on the gap type:
   ...
   - **Community counter-evidence gap** (`type: "community-counter-evidence"` from corpus-critic, OR the gap is about real-user pain / lived experience / contested vendor claim) → `mcp__reddit__reddit_search` + `mcp__reddit__reddit_search_subreddit` (name the obvious subreddit). For a load-bearing thread also hint `mcp__reddit__reddit_get_post(post_id)` to pull the full comment tree. Always tell the fetcher to persist Reddit URLs via `{hpr_path} fetch --tag reddit`.
   ...
   - **Recursive multi-page crawl gap** (the gap spans many pages of one domain that doesn't expose a sitemap — forum threads, paginated regulator portals) → `mcp__firecrawl__firecrawl_crawl(url, maxDepth: 2, limit: 30)` then poll with `firecrawl_check_crawl_status`. **Credit-heavy** — cap `limit` aggressively at the gap-fill stage (this is a surgical fill, not a width sweep).
